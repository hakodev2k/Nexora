...
## 1. Scope
Translate the current FX-28 feature behavior into a coherent Nexora UI. Reference products do not add scope automatically.

## 2. Requirement sources
- `docs/features/28-vault.md`
...
- Desktop three-pane category/list/detail.
- Protected fields masked; Reveal/Copy explicit.
- Reveal remasks after delegated window/context change.
- Safe metadata only in search.
- Recovery/import/export/recent-auth remains Q-dependent.
...
Desktop: Categories/Folders | Item list | Item detail. Mobile becomes category → list → detail.

Protected field row: Label | •••••••• | Reveal | Copy. Reveal does not copy; Copy does not reveal.

Create chooses immutable type. Generator may fill a field but persists only on Save.

Historical protected payload remains masked until separately permitted reveal.

Support/Emergency has no Reveal, Copy, Export or plaintext preview affordance.