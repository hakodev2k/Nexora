...
## 1. Scope
Translate the current FX-20 feature behavior into a coherent Nexora UI. Reference products do not add scope automatically.

## 2. Requirement sources
- `docs/features/20-documents.md`
...
- Documents default Grid and direct-location scope.
- Every create explicitly chooses Type and EditorMode.
- Manual Save is prominent; no autosave imitation.
- Module-local document tree avoids polluting global sidebar.
- Archived is read-only with cohort-aware tree behavior.
- Share is explicit and does not include children.
...
Desktop: Global sidebar | Documents navigation | Main library.

Documents navigation contains Library and Archived plus bounded Folder/page hierarchy.

Create Page explicitly asks DocumentType and EditorMode with no default/remembered choice. Both become immutable.

Block Editor top actions retain lifecycle state, Share when eligible, Version history, Save, More.

Markdown desktop may use Editor/Preview split; mobile uses Edit/Preview toggle.

Version History opens read-only version preview; Restore creates a new current version.

Archive parent dialog previews affected children and explains archive cohort. Archived state shows persistent Read-only banner and supports history, eligible existing share, Unarchive when allowed, Move to Trash.

Restore explains parent/dependency conflicts rather than offering misleading success.