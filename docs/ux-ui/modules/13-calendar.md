...
## 1. Scope
Translate the current FX-13 feature behavior into a coherent Nexora UI. Reference products do not add scope automatically.

## 2. Requirement sources
- `docs/features/13-calendar.md`
...
- Day default with explicit view switch.
- Source and state are separate visual concepts.
- Task projection opens Task detail and is not editable in Calendar.
- No drag/resize editing in R1.
- Canceled event remains visible terminal.
...
Header: Today · previous/next · current range · Day/Week/Month/Agenda · + Event.

Manual Event vs Task projection uses icon/label/border in addition to color.

ICS import: Choose file → Parse/Preview → Review skipped/duplicate → Import valid → Result report.

Mobile prefers Day/Agenda for scanning; Month can drill into a day.