...
## 1. Scope
Translate the current FX-30 feature behavior into a coherent Nexora UI. Reference products do not add scope automatically.

## 2. Requirement sources
- `docs/features/30-shopee-price-tracking.md`
...
- Detail prioritizes current price/variant/freshness/history/rules.
- Chart never mixes variant/currency/source.
- Failure retains last valid observation with stale state.
- Polling/provider/price definition remains Proposed.
...
Tracked row: product title, exact variant, current valid price/currency, last checked and freshness.

Detail order: identity → current price/stock truth → provider state → history chart → rules → observations.

Add watch: Paste URL → Resolve → confirm exact variant → preview normalized product → Watch.

Provider outage retains last valid observation and never emits fake 0/OutOfStock.