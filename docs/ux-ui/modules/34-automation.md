...
## 1. Scope
Translate the current FX-34 feature behavior into a coherent Nexora UI. Reference products do not add scope automatically.

## 2. Requirement sources
- `docs/features/34-automation-and-scheduler.md`
...
- Structured Trigger→Conditions→Actions baseline until Q-07.
- Enabled state separate from run result.
- Run history supports eligible retry.
...
Automation list: Name, Enabled, Trigger summary, Last run/result, Next run.

Builder baseline is structured rather than assuming free graph canvas. Each action shows target module/resource and outbound projection.

Run Detail uses step timeline with safe output summary. If earlier steps succeeded before failure, say partial side effects may already exist; never imply rollback without guarantee.

If bounded DAG is later approved, keep a keyboard/non-canvas representation.