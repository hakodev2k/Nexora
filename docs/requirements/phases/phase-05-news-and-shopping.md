...
**Phase ID:** `NX-PH-05`  
**Outcome:** User theo dõi nguồn tin và sản phẩm/giá, lưu nội dung cần đọc/mua và nhận cảnh báo đáng tin cậy khi điều kiện đã cấu hình xảy ra.  
**Depends on:** Notifications, scheduler/jobs, safe external HTTP policy, unified Read Later, search and files.
...
| `P05-FED-004` | P0 | Same normalized source của cùng owner xử lý duplicate theo policy; source giữa users không chia sẻ ownership ngầm. | Add retry không duplicate; cached public fetch có tenant-safe projections. |
...
| ID | Pri | Requirement | Acceptance criteria |
|---|---:|---|---|
| `P05-PRD-001` | P0 | User thêm Shopee URL; adapter parse/normalize listing identity và reject unsupported/invalid URL. | Equivalent URL không duplicate trong cùng owner ngoài explicit choice. |
| `P05-PRD-002` | P0 | Tracked product có owner, provider/listing ID, URL, name, shop, image URL policy, selected variant, currency, current/previous/lowest, last checked/status. | Unknown fields are null/labeled, never fabricated; owner set server-side. |
...
| ID | Pri | Requirement | Acceptance criteria |
|---|---:|---|---|
| `P05-ALT-001` | P0 | Alert rule có owner/product, type, threshold/currency, active state, cooldown/re-arm policy và channel preference. | Invalid threshold/currency blocked; cross-owner product blocked. |
...
- Feed subscription/read state, tracking rules, wishlist/purchase/history are private user data.
...
7. User A/Admin-no-scope cannot see User B feeds, reading history, trackers, alerts or purchase records.
