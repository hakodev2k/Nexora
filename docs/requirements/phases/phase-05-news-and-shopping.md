...
**Phase ID:** `NX-PH-05`  
**Version:** `1.1-draft`  
**Outcome:** User theo dõi nguồn tin và sản phẩm/giá trong Space được module hỗ trợ, lưu nội dung cần đọc/mua và nhận cảnh báo đáng tin cậy khi điều kiện đã cấu hình xảy ra.  
**Depends on:** Module Platform/Space access, notifications, scheduler/jobs, safe external HTTP policy, unified Read Later, search and files.
...
- `DEC-PRD-019` phải xác định News/Shopping hỗ trợ Personal, Workspace hay cả hai; manifest phải khai báo trước khi enable.
- Read/unread, follow, alert delivery và notification preference vẫn per-user dù source/product thuộc Workspace.
- Background fetch có thể dùng cache public chung nhưng subscription/tracker/rule/result projection luôn re-check Space/module/member access.
...
| `P05-FED-004` | P0 | Same normalized source trong cùng Space xử lý duplicate theo policy; source giữa Space không chia sẻ ownership ngầm. | Add retry không duplicate; cached public fetch có Space-safe projections. |
...
| ID | Pri | Requirement | Acceptance criteria |
|---|---:|---|---|
| `P05-PRD-001` | P0 | User thêm Shopee URL; adapter parse/normalize listing identity và reject unsupported/invalid URL. | Equivalent URL không duplicate trong cùng Space ngoài explicit choice. |
| `P05-PRD-002` | P0 | Tracked product có owning Space, creator, provider/listing ID, URL, name, shop, image URL policy, selected variant, currency, current/previous/lowest, last checked/status. | Unknown fields are null/labeled, never fabricated; Space/creator set server-side. |
...
| ID | Pri | Requirement | Acceptance criteria |
|---|---:|---|---|
| `P05-ALT-001` | P0 | Alert rule có Space/product, creator, type, threshold/currency, active state, cooldown/re-arm policy và channel preference. | Invalid threshold/currency blocked; cross-Space product blocked. |
...
- Feed subscription/tracking resource theo owning Space; read state, follow, notification preference và delivery là per-user private data.
- Public provider data cache may be shared internally only if per-user state/access remains isolated.
- Admin namespace `news.*`, `shopping.*`; `view` without `access_all` cannot see user subscriptions/tracking.
...
- Workspace member/module access bị revoke sau khi job queued nhưng trước fetch/evaluate/deliver.
...
7. User/Workspace A và Admin-no-scope không thấy feeds, read state, trackers, alerts hoặc purchase records của User/Workspace B.
8. Removed Member hoặc disabled Workspace module không nhận alert, mở tracker hay suy ra aggregate qua stale job/cache.
...
- Manifest `supportedSpaces`, cross-workspace isolation và per-user state trên Workspace resource được test.
