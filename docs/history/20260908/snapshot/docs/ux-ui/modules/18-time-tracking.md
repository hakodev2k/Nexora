# FX-18 — Time Tracking — UX/UI Specification
...
One personal running timer plus manually entered time, overlap-aware gross reports and history.

## 2. Requirement sources

- [FX-18 feature](../../features/18-time-tracking.md) — business fields, rules and AC remain authoritative.
...
Source identifiers reviewed: `DEC-GOV-001`, `FX-18-BR-001`, `FX-18-BR-002`, `FX-18-BR-003`, `FX-18-BR-004`, `FX-18-BR-005`, `FX-18-AC-001`, `FX-18-AC-002`, `FX-18-AC-003`

## 3. Reference products

- [Toggl Track](https://support.toggl.com/en-us/article/creating-a-time-entry-wg8nug/) — checked2026-09-07. Evidence limit: Official documentation text; no Toggl workspace integration.
...
- FX18-S01 — Timer: WORKBENCH profile.
- FX18-S02 — Time entries: BROWSE profile.
- FX18-S03 — Entry form: FORM profile.
- FX18-S04 — Time reports / history: HISTORY profile.
...
| --- | --- | --- | --- | --- |
| FX18-S01 | Timer | /time | Server-based elapsed; optional source; description; one running entry banner | Start / Stop |
| FX18-S02 | Time entries | /time/entries | Start; End; duration; description; Project/Task; category | Manual entry |
| FX18-S03 | Entry form | /time/entries/new; /time/entries/:entryId/edit | Start/End; description; Project/Task; tags/category | Save entry |
| FX18-S04 | Time reports / history | /time/reports; /time/entries/:entryId/history | Date/source aggregates or immutable entry revisions; exact duration | Inspect details |
...
**Primary:** Time Tracking → Start timer → Stop → saved entry; Manual entry → overlap preview → Save; previous entry Continue creates new record.
...
| --- | --- |
| Purpose / profile | WORKBENCH — Server-based elapsed; optional source; description; one running entry banner |
| Entry / proposed route | /time; module/source navigation or authorized deep link. |
| Header / layout | Screen title: Timer. Shared workbench profile; header → controls → declared content → feedback. |
| Primary action | Start / Stop; available only when section15/context permits; otherwise explain lifecycle/policy. |
| Secondary actions | Manual entry; recent entries. Back/Cancel always has authorized fallback. |
| Content regions / fields | Server-based elapsed; optional source; description; one running entry banner |
| Search / filters / sorting / pagination | No active-timer pagination. Controls not listed here are N/A, not implicit new fields. |
| Interaction / validation overrides | Reload/background tab derives elapsed from server time. Competing tab start returns existing timer guidance, not two clocks. |
...
| Desktop / tablet / mobile | UX-10 workbench profile. Keep 'Timer' context and required fields; mobile prioritizes first declared identity and status/time/value, remaining fields detail/expand.  |
...
### FX18-S02 — Time entries

| Dimension | Specification |
| --- | --- |
| Purpose / profile | BROWSE — Start; End; duration; description; Project/Task; category |
| Entry / proposed route | /time/entries; module/source navigation or authorized deep link. |
| Header / layout | Screen title: Time entries. Shared browse profile; header → controls → declared content → feedback. |
| Primary action | Manual entry; available only when section15/context permits; otherwise explain lifecycle/policy. |
| Secondary actions | Edit; Continue; History; Trash. Back/Cancel always has authorized fallback. |
| Content regions / fields | Start; End; duration; description; Project/Task; category |
| Search / filters / sorting / pagination | Date/source/category; description; Start DESC;25/page. Controls not listed here are N/A, not implicit new fields. |
| Interaction / validation overrides | Continue new entry, no overwrite past End. Gross sum can exceed wall-clock period if overlaps, explicitly label. |
| Loading / empty / error | UX-15A state contract. Empty: declared data absent; no matches: clear listed query controls; fetch error: safe retry. These are distinct, no error-as-empty. |
| Disabled / readonly / Archived / Trash | UX-15A plus exact section15 matrix. No lifecycle in source = N/A. Do not render unauthorized payload behind disabled controls. |
| Conflict / destructive | Current revision and parent guards, section16 dialogs. Readonly surfaces only refresh, never forced write. |
| Desktop / tablet / mobile | UX-10 browse profile. Keep 'Time entries' context and required fields; mobile prioritizes first declared identity and status/time/value, remaining fields detail/expand.  |
| Keyboard / accessibility | UX-11 and profile: visible focus, labeled controls, no drag/hover-only action, status text; dialog focus trap/return; charts/table values reachable. |
| Exit / return | Back/Cancel → actual invoking screen with view/filter/date/scroll restored; direct deep link → module root or safe Home/Admin landing; dirty form guard and revoked-data clear take precedence. |
...
| Dimension | Specification |
| --- | --- |
| Purpose / profile | FORM — Start/End; description; Project/Task; tags/category |
| Entry / proposed route | /time/entries/new; /time/entries/:entryId/edit; module/source navigation or authorized deep link. |
| Header / layout | Screen title: Entry form. Shared form profile; header → controls → declared content → feedback. |
| Primary action | Save entry; available only when section15/context permits; otherwise explain lifecycle/policy. |
| Secondary actions | Cancel. Back/Cancel always has authorized fallback. |
| Content regions / fields | Start/End; description; Project/Task; tags/category |
| Search / filters / sorting / pagination | Timezone visible; source picker25/page. Controls not listed here are N/A, not implicit new fields. |
| Interaction / validation overrides | Overlap confirmation shows intersecting entries. Terminal source cannot be reopened by manual timer association. |
| Loading / empty / error | UX-15A state contract. Empty: declared data absent; no matches: clear listed query controls; fetch error: safe retry. These are distinct, no error-as-empty. |
| Disabled / readonly / Archived / Trash | UX-15A plus exact section15 matrix. No lifecycle in source = N/A. Do not render unauthorized payload behind disabled controls. |
| Conflict / destructive | Current revision and parent guards, section16 dialogs. Readonly surfaces only refresh, never forced write. |
| Desktop / tablet / mobile | UX-10 form profile. Keep 'Entry form' context and required fields; mobile prioritizes first declared identity and status/time/value, remaining fields detail/expand.  |
| Keyboard / accessibility | UX-11 and profile: visible focus, labeled controls, no drag/hover-only action, status text; dialog focus trap/return; charts/table values reachable. |
| Exit / return | Back/Cancel → actual invoking screen with view/filter/date/scroll restored; direct deep link → module root or safe Home/Admin landing; dirty form guard and revoked-data clear take precedence. |

### FX18-S04 — Time reports / history

| Dimension | Specification |
| --- | --- |
| Purpose / profile | HISTORY — Date/source aggregates or immutable entry revisions; exact duration |
| Entry / proposed route | /time/reports; /time/entries/:entryId/history; module/source navigation or authorized deep link. |
| Header / layout | Screen title: Time reports / history. Shared history profile; header → controls → declared content → feedback. |
| Primary action | Inspect details; available only when section15/context permits; otherwise explain lifecycle/policy. |
| Secondary actions | Back; change range. Back/Cancel always has authorized fallback. |
| Content regions / fields | Date/source aggregates or immutable entry revisions; exact duration |
| Search / filters / sorting / pagination | Date/category/source range; chronological details25/page. Controls not listed here are N/A, not implicit new fields. |
| Interaction / validation overrides | Charts have tabular durations; no billable/payroll/currency controls. |
| Loading / empty / error | UX-15A state contract. Empty: declared data absent; no matches: clear listed query controls; fetch error: safe retry. These are distinct, no error-as-empty. |
| Disabled / readonly / Archived / Trash | UX-15A plus exact section15 matrix. No lifecycle in source = N/A. Do not render unauthorized payload behind disabled controls. |
| Conflict / destructive | Current revision and parent guards, section16 dialogs. Readonly surfaces only refresh, never forced write. |
| Desktop / tablet / mobile | UX-10 history profile. Keep 'Time reports / history' context and required fields; mobile prioritizes first declared identity and status/time/value, remaining fields detail/expand.  |
| Keyboard / accessibility | UX-11 and profile: visible focus, labeled controls, no drag/hover-only action, status text; dialog focus trap/return; charts/table values reachable. |
| Exit / return | Back/Cancel → actual invoking screen with view/filter/date/scroll restored; direct deep link → module root or safe Home/Admin landing; dirty form guard and revoked-data clear take precedence. |

## 11. Forms and validation

Start/End required for manual; End>Start; description<=2000 optional; category/tags optional; same-owner Project/Task refs optional and consistent. One active timer/User. Manual overlap warns/confirm. Entry correction history required.
...
Entries newest Start first; date/Project/Task/category filters; description search;25/page. Reports day/week/custom with gross duration label and overlap warning.
...
| --- | --- | --- |
| No timer | Start new timer or manual entry | No browser-local-only authority |
| Running | Stop; inspect elapsed | Start second requires stop current first |
| Stopped | Edit with history; Continue creates new entry; Trash | Source-locked Task cannot be continued |
| Source Project terminal/Task Trash | Timer stopped at transition instant | Do not resume locked source |
| Trash | Restore under source/lifecycle rules; purge | No financial posting |
...
| --- | --- |
| [productivity.TimeEntry](../../design-database/05-productivity-calendar.md#productivity-timeentry) | Personal elapsed-time record, never payroll ledger; Technical decision |
| [productivity.TimeEntryVersion](../../design-database/05-productivity-calendar.md#productivity-timeentryversion) | Immutable time-entry edit history; Technical decision |
