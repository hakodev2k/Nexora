# FX-08 — Trash / Activity / Audit — UX/UI Specification

> **Current decision amendment — 2026-09-07:** No Account/Vault purge; Vault restore uses SuperAdmin Recovery. Other modules keep existing Trash/manual purge. [Normative PO decisions](../../requirements/10-owner-decisions-20260907.md). Conflicting older proposal paragraphs below are historical; current field/action overrides are in the linked delta. Docs-only.
...
Separate owner Trash, resource Activity, immutable History and operator Audit.

## 2. Requirement sources

- [FX-08 feature](../../features/08-trash-activity-and-audit.md) — business fields, rules and AC remain authoritative.
...
Source identifiers reviewed: `DEC-GOV-001`, `FX-08-BR-001`, `FX-08-BR-002`, `FX-08-BR-003`, `FX-08-BR-004`, `FX-08-BR-005`, `FX-08-BR-006`, `FX-08-AC-001`, `FX-08-AC-002`, `FX-08-AC-003`, `P01-PLT-001`, `P01-PLT-002`

## 3. Reference products

- [Notion](https://www.notion.com/help/duplicate-delete-and-restore-content) — checked2026-09-07. Evidence limit: Official help text; no live deletion performed.
...
- FX08-S01 — Trash: BROWSE profile.
- FX08-S02 — Restore / purge preview: DIALOG profile.
- FX08-S03 — Resource activity: HISTORY profile.
- FX08-S04 — Audit log: BROWSE profile.
...
| --- | --- | --- | --- | --- |
| FX08-S01 | Trash | /trash | Safe title; module/type; deleted time; batch/root label; blocked reason | Preview restore |
| FX08-S02 | Restore / purge preview | /trash/operations/new | Affected root/children; original location/status; blocked or pinned references; irreversible count | Restore / Delete permanently |
| FX08-S03 | Resource activity | /activity?resource=:opaqueId | When; actor Self/System; semantic change; owner-only reason if applicable | Open relevant version |
| FX08-S04 | Audit log | /admin/audit | Occurred; actor reference; subject reference; action; outcome; correlation | Inspect audit event |
...
**Primary:** Trash → select root → dependency preview → Restore or Delete permanently; Activity/History accessed from resource; Audit only authorized admin.
...
| --- | --- |
| Purpose / profile | BROWSE — Safe title; module/type; deleted time; batch/root label; blocked reason |
| Entry / proposed route | /trash; module/source navigation or authorized deep link. |
| Header / layout | Screen title: Trash. Shared browse profile; header → controls → declared content → feedback. |
| Primary action | Preview restore; available only when section15/context permits; otherwise explain lifecycle/policy. |
| Secondary actions | Delete permanently selected. Back/Cancel always has authorized fallback. |
| Content regions / fields | Safe title; module/type; deleted time; batch/root label; blocked reason |
| Search / filters / sorting / pagination | Title/module/type/date; deleted DESC;25/page. Controls not listed here are N/A, not implicit new fields. |
| Interaction / validation overrides | Root groups display membership count. Already separately trashed child shown separate batch, not silently included in restore. |
...
| Desktop / tablet / mobile | UX-10 browse profile. Keep 'Trash' context and required fields; mobile prioritizes first declared identity and status/time/value, remaining fields detail/expand.  |
...
### FX08-S02 — Restore / purge preview

| Dimension | Specification |
| --- | --- |
| Purpose / profile | DIALOG — Affected root/children; original location/status; blocked or pinned references; irreversible count |
| Entry / proposed route | /trash/operations/new; module/source navigation or authorized deep link. |
| Header / layout | Screen title: Restore / purge preview. Named modal with preview/reason and footer actions. |
| Primary action | Restore / Delete permanently; available only when section15/context permits; otherwise explain lifecycle/policy. |
| Secondary actions | Cancel. Back/Cancel always has authorized fallback. |
| Content regions / fields | Affected root/children; original location/status; blocked or pinned references; irreversible count |
| Search / filters / sorting / pagination | Scrollable dependency tree/table. Controls not listed here are N/A, not implicit new fields. |
| Interaction / validation overrides | Revalidate at confirm; aggregate atomic restore, heterogeneous bulk per-item report. No promise recoverable after purge. |
| Loading / empty / error | UX-15A state contract. Empty: declared data absent; no matches: clear listed query controls; fetch error: safe retry. These are distinct, no error-as-empty. |
| Disabled / readonly / Archived / Trash | UX-15A plus exact section15 matrix. No lifecycle in source = N/A. Do not render unauthorized payload behind disabled controls. |
| Conflict / destructive | Current revision and parent guards, section16 dialogs. Readonly surfaces only refresh, never forced write. |
| Desktop / tablet / mobile | UX-10 dialog profile. Keep 'Restore / purge preview' context and required fields; mobile prioritizes first declared identity and status/time/value, remaining fields detail/expand.  |
| Keyboard / accessibility | UX-11 and profile: visible focus, labeled controls, no drag/hover-only action, status text; dialog focus trap/return; charts/table values reachable. |
| Exit / return | Back/Cancel → actual invoking screen with view/filter/date/scroll restored; direct deep link → module root or safe Home/Admin landing; dirty form guard and revoked-data clear take precedence. |

### FX08-S03 — Resource activity

| Dimension | Specification |
| --- | --- |
| Purpose / profile | HISTORY — When; actor Self/System; semantic change; owner-only reason if applicable |
| Entry / proposed route | /activity?resource=:opaqueId; module/source navigation or authorized deep link. |
| Header / layout | Screen title: Resource activity. Shared history profile; header → controls → declared content → feedback. |
| Primary action | Open relevant version; available only when section15/context permits; otherwise explain lifecycle/policy. |
| Secondary actions | Back to source. Back/Cancel always has authorized fallback. |
| Content regions / fields | When; actor Self/System; semantic change; owner-only reason if applicable |
| Search / filters / sorting / pagination | Action/date filter; newest;25/page. Controls not listed here are N/A, not implicit new fields. |
| Interaction / validation overrides | Activity safe summaries are not full content versions; share viewer never reaches route. |
| Loading / empty / error | UX-15A state contract. Empty: declared data absent; no matches: clear listed query controls; fetch error: safe retry. These are distinct, no error-as-empty. |
| Disabled / readonly / Archived / Trash | UX-15A plus exact section15 matrix. No lifecycle in source = N/A. Do not render unauthorized payload behind disabled controls. |
| Conflict / destructive | Current revision and parent guards, section16 dialogs. Readonly surfaces only refresh, never forced write. |
| Desktop / tablet / mobile | UX-10 history profile. Keep 'Resource activity' context and required fields; mobile prioritizes first declared identity and status/time/value, remaining fields detail/expand.  |
| Keyboard / accessibility | UX-11 and profile: visible focus, labeled controls, no drag/hover-only action, status text; dialog focus trap/return; charts/table values reachable. |
| Exit / return | Back/Cancel → actual invoking screen with view/filter/date/scroll restored; direct deep link → module root or safe Home/Admin landing; dirty form guard and revoked-data clear take precedence. |
...
| Dimension | Specification |
| --- | --- |
| Purpose / profile | BROWSE — Occurred; actor reference; subject reference; action; outcome; correlation |
| Entry / proposed route | /admin/audit; module/source navigation or authorized deep link. |
| Header / layout | Screen title: Audit log. Shared browse profile; header → controls → declared content → feedback. |
| Primary action | Inspect audit event; available only when section15/context permits; otherwise explain lifecycle/policy. |
| Secondary actions | Filter; Back to Admin. Back/Cancel always has authorized fallback. |
| Content regions / fields | Occurred; actor reference; subject reference; action; outcome; correlation |
| Search / filters / sorting / pagination | Actor/action/outcome/date; newest;25/page. Controls not listed here are N/A, not implicit new fields. |
| Interaction / validation overrides | Admin audit rights do not grant underlying personal resource access; secret/body excluded. |
| Loading / empty / error | UX-15A state contract. Empty: declared data absent; no matches: clear listed query controls; fetch error: safe retry. These are distinct, no error-as-empty. |
| Disabled / readonly / Archived / Trash | UX-15A plus exact section15 matrix. No lifecycle in source = N/A. Do not render unauthorized payload behind disabled controls. |
| Conflict / destructive | Current revision and parent guards, section16 dialogs. Readonly surfaces only refresh, never forced write. |
| Desktop / tablet / mobile | UX-10 browse profile. Keep 'Audit log' context and required fields; mobile prioritizes first declared identity and status/time/value, remaining fields detail/expand.  |
| Keyboard / accessibility | UX-11 and profile: visible focus, labeled controls, no drag/hover-only action, status text; dialog focus trap/return; charts/table values reachable. |
| Exit / return | Back/Cancel → actual invoking screen with view/filter/date/scroll restored; direct deep link → module root or safe Home/Admin landing; dirty form guard and revoked-data clear take precedence. |

## 11. Forms and validation

Permanent deletion requires explicit irreversible warning and resource-name confirmation for aggregate/multi-item purge. Reason required only business rules (backward Task, overridden completion, emergency), not every small edit. No retention input overriding approved indefinite Trash.
...
Trash search safe title; module/type/deleted-time filter; Deleted DESC25/page. Activity per resource newest first. Audit safe actor/action/outcome/time filters, occurred DESC; no content search.
...
| --- | --- | --- |
| Trash eligible | Preview and Restore; Delete permanently with dependency checks | No edit/share/download private payload by default |
| Task parent Trash | Restore parent batch first | Individual Task restore never available |
| Task parent terminal | Inspect safe Trash metadata; approved purge | Restore forbidden |
| History version | Readonly inspect; only source-approved Restore | No generic permission to reopen terminal resource |
| Audit | Authorized readonly query | Owner Delete notification/Trash never deletes Audit |
...
| --- | --- | --- | --- |
| Restore / purge preview | Affected root/children; original location/status; blocked or pinned references; irreversible count | Restore / Delete permanently / Cancel | Revalidate at confirm; aggregate atomic restore, heterogeneous bulk per-item report. No promise recoverable after purge. |
...
| [security.ActivityEvent](../../design-database/03-security-sharing.md#security-activityevent) | User-visible resource activity, separate from immutable audit; Technical decision |
| [operations.TrashBatch](../../design-database/04-files-jobs-notifications.md#operations-trashbatch) | Atomic deletion cohort for aggregate trees; Technical decision |
| [operations.TrashMember](../../design-database/04-files-jobs-notifications.md#operations-trashmember) | Frozen tree snapshot membership; Technical decision |
