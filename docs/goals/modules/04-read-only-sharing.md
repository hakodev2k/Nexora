# FX-04 — Read-only Sharing Engine

Product phase: **P01** · Delivery: **RM06** · Phase 8/RM16–RM22 kiểm chứng tích hợp và phát hành.

**Trạng thái:** Specified; chưa approve implementation, chưa Implemented/Verified. Goals có điều kiện không là quyền code. FX-04 phải đọc cùng [goals toàn hệ thống](../01-system-goals.md), [hợp đồng dùng goals](../README.md) và sources hiện hành.

## Mục tiêu và bằng chứng chấp nhận

| Goal ID | Outcome phải giữ khi code | Bằng chứng test tối thiểu khi thực thi |
| --- | --- | --- |
| NXG-FX04-G01 | Owner tạo link chỉ-đọc đúng resource, audience và projection được duyệt. | Các mode được source cho phép, sai account, expired/revoked và file URL đều test; không owner mutation. |
| NXG-FX04-G02 | Project live share và Documents lifecycle có semantics riêng. | Project cập nhật Task details live nhưng không history/reason; page không tự share children; Draft suspend, Archived giữ link hợp lệ. |
| NXG-FX04-G03 | Sharing off/source delete làm token cũ mất hiệu lực vĩnh viễn. | Restore/re-enable vẫn 404 token cũ, token mới khác; deletion cohort invalidates child links cùng commit. |

## Ownership, dependencies và giới hạn

- Contracts phụ thuộc: Resource providers, Files, owner access, lifecycle. Chỉ truy cập module khác qua contract, không trực tiếp table/DbContext/private store.
- Phạm vi/gate: P-H03: sensitive share; P-H04: Vault portability/projection.
...
- [Feature/BR/AC nguồn](../../features/04-read-only-sharing.md) — đọc toàn bộ field/state/validation và trace requirements tại đó.
- [Action contracts](../../action-catalog/modules/04-sharing.md) — exact action keys, contexts, prerequisites, status/gate; catalog có 6 rows: Resolved delegated: 6. Đây là inventory, không phải coverage đã pass.
- [UX/screens](../../ux-ui/modules/04-sharing.md) — luồng màn hình, disabled/loading/error và interaction contracts.
...
- AC hiện hành cần kế thừa: `FX-04-AC-001`, `FX-04-AC-002`, `FX-04-AC-003`. Cộng source requirement AC, POAC liên quan và [cross-module journeys](../04-cross-module-verification.md); danh sách này không thay test plan đầy đủ.
