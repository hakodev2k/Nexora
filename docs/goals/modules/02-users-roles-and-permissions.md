# FX-02 — Users, Roles và Action Permissions

Product phase: **P01** · Delivery: **RM05** · Phase 8/RM16–RM22 kiểm chứng tích hợp và phát hành.

**Trạng thái:** Specified; chưa approve implementation, chưa Implemented/Verified. Goals có điều kiện không là quyền code. FX-02 phải đọc cùng [goals toàn hệ thống](../01-system-goals.md), [hợp đồng dùng goals](../README.md) và sources hiện hành.

## Mục tiêu và bằng chứng chấp nhận

| Goal ID | Outcome phải giữ khi code | Bằng chứng test tối thiểu khi thực thi |
| --- | --- | --- |
| NXG-FX02-G01 | Quản trị role/action bằng quyền tường minh, không tự nâng quyền. | Admin-grantable contexts: absent = deny, explicit Deny thắng Allow; User SELF hợp lệ theo owner/module baseline vẫn được phép, không cần Admin grants; Admin không mượn Self context để vượt Deny; mutation response không rò read DTO. |
| NXG-FX02-G02 | Giữ ít nhất một SuperAdmin hoạt động. | Hai lệnh demote/disable cạnh tranh không cùng thành công làm mất SuperAdmin cuối. |
| NXG-FX02-G03 | Đổi quyền có preview, revision, audit và thu hồi hiệu lực. | Tab cũ, stale preview, bulk partial failure và request sau revoke cho kết quả đúng, không private domain payload. |

## Ownership, dependencies và giới hạn

- Contracts phụ thuộc: Identity, authorization catalog, Audit, SQL concurrency. Chỉ truy cập module khác qua contract, không trực tiếp table/DbContext/private store.
...
- [Feature/BR/AC nguồn](../../features/02-users-roles-and-permissions.md) — đọc toàn bộ field/state/validation và trace requirements tại đó.
- [Action contracts](../../action-catalog/modules/02-access.md) — exact action keys, contexts, prerequisites, status/gate; catalog có 9 rows: Resolved delegated: 9. Đây là inventory, không phải coverage đã pass.
- [UX/screens](../../ux-ui/modules/02-users-roles-permissions.md) — luồng màn hình, disabled/loading/error và interaction contracts.
...
- AC hiện hành cần kế thừa: `FX-02-AC-001`, `FX-02-AC-002`, `FX-02-AC-003`. Cộng source requirement AC, POAC liên quan và [cross-module journeys](../04-cross-module-verification.md); danh sách này không thay test plan đầy đủ.
