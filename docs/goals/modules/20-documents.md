# FX-20 — Documents, Note và Knowledge Pages

Product phase: **P03** · Delivery: **RM09** · Phase 8/RM16–RM22 kiểm chứng tích hợp và phát hành.

**Trạng thái:** Specified; chưa approve implementation, chưa Implemented/Verified. Goals có điều kiện không là quyền code. FX-20 phải đọc cùng [goals toàn hệ thống](../01-system-goals.md), [hợp đồng dùng goals](../README.md) và sources hiện hành.

## Mục tiêu và bằng chứng chấp nhận

| Goal ID | Outcome phải giữ khi code | Bằng chứng test tối thiểu khi thực thi |
| --- | --- | --- |
| NXG-FX20-G01 | Documents giữ immutable type/editor/folder/parent và cấu trúc hai cấp. | Create bắt chọn type/mode; forged template/restore không đổi membership; duplicate title hợp lệ; một Tag, icon hoặc cover. |
| NXG-FX20-G02 | Manual Save/version và Archive/Trash theo cohort bảo toàn nội dung. | Retry một version, distinct Save tạo version; Unarchive không revive child archived riêng/Trash; binary lịch sử đúng. |
| NXG-FX20-G03 | Listing, sharing và DOCX/MD tuân contract riêng. | Local search Title/Tag tại vị trí trực tiếp; Published vẫn private; share không children; lossy preview và XSS fixtures pass. |

## Ownership, dependencies và giới hạn

- Contracts phụ thuộc: Files, tags, lifecycle, Sharing, format providers. Chỉ truy cập module khác qua contract, không trực tiếp table/DbContext/private store.
...
- [Feature/BR/AC nguồn](../../features/20-documents.md) — đọc toàn bộ field/state/validation và trace requirements tại đó.
- [Action contracts](../../action-catalog/modules/20-documents.md) — exact action keys, contexts, prerequisites, status/gate; catalog có 27 rows: Resolved delegated: 27. Đây là inventory, không phải coverage đã pass.
- [UX/screens](../../ux-ui/modules/20-documents.md) — luồng màn hình, disabled/loading/error và interaction contracts.
...
- AC hiện hành cần kế thừa: `FX-20-AC-001`, `FX-20-AC-002`, `FX-20-AC-003`, `FX-20-AC-004`, `FX-20-AC-005`. Cộng source requirement AC, POAC liên quan và [cross-module journeys](../04-cross-module-verification.md); danh sách này không thay test plan đầy đủ.
