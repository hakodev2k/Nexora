# FX-03 — Module Platform và Module Manager

Product phase: **P01** · Delivery: **RM06** · Phase 8/RM16–RM22 kiểm chứng tích hợp và phát hành.

**Trạng thái:** Specified; chưa approve implementation, chưa Implemented/Verified. Goals có điều kiện không là quyền code. FX-03 phải đọc cùng [goals toàn hệ thống](../01-system-goals.md), [hợp đồng dùng goals](../README.md) và sources hiện hành.

## Mục tiêu và bằng chứng chấp nhận

| Goal ID | Outcome phải giữ khi code | Bằng chứng test tối thiểu khi thực thi |
| --- | --- | --- |
| NXG-FX03-G01 | Module tích hợp qua manifest và contracts có version. | Duplicate action/route, dependency cycle, missing handler hoặc migration lỗi không Ready. |
| NXG-FX03-G02 | Enablement phản ánh installed, active scope, system/user và action. | Paused/uninstalled không được default-on; direct API/job/search/widget bị chặn khi disabled. |
| NXG-FX03-G03 | Disable và upgrade bảo toàn dữ liệu và khả năng phục hồi. | Re-enable không nhân records/reminders hoặc hồi sinh grant/link đã revoke; upgrade failure có evidence. |

## Ownership, dependencies và giới hạn

- Contracts phụ thuộc: Platform Contracts, migration journal, access evaluator. Chỉ truy cập module khác qua contract, không trực tiếp table/DbContext/private store.
...
- [Feature/BR/AC nguồn](../../features/03-module-platform.md) — đọc toàn bộ field/state/validation và trace requirements tại đó.
- [Action contracts](../../action-catalog/modules/03-modules.md) — exact action keys, contexts, prerequisites, status/gate; catalog có 10 rows: Resolved delegated: 10. Đây là inventory, không phải coverage đã pass.
- [UX/screens](../../ux-ui/modules/03-module-platform.md) — luồng màn hình, disabled/loading/error và interaction contracts.
...
- AC hiện hành cần kế thừa: `FX-03-AC-001`, `FX-03-AC-002`, `FX-03-AC-003`. Cộng source requirement AC, POAC liên quan và [cross-module journeys](../04-cross-module-verification.md); danh sách này không thay test plan đầy đủ.
