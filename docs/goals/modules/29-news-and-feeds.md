# FX-29 — News, RSS và Topic Watch

Product phase: **P05** · Delivery: **RM13** · Phase 8/RM16–RM22 kiểm chứng tích hợp và phát hành.

**Trạng thái:** Specified; chưa approve implementation, chưa Implemented/Verified. Goals có điều kiện không là quyền code. FX-29 phải đọc cùng [goals toàn hệ thống](../01-system-goals.md), [hợp đồng dùng goals](../README.md) và sources hiện hành.

## Mục tiêu và bằng chứng chấp nhận

| Goal ID | Outcome phải giữ khi code | Bằng chứng test tối thiểu khi thực thi |
| --- | --- | --- |
| NXG-FX29-G01 | News reader dùng dữ liệu đã lưu, state riêng mỗi owner. | Read/unread/watermark đúng; unfollow không xóa saved-later; article update không reset read state. |
| NXG-FX29-G02 | Ingestion chỉ hoạt động sau phạm vi outbound được duyệt. | Trước approval không fetch/manual refresh/poll; sau approval XXE/SSRF/redirect/size/timeout fixtures phải pass. |
| NXG-FX29-G03 | Topic watch và nguồn stale không báo tin giả hoặc spam. | Dedupe watch+new article; query edit không bắn history; lastSuccess/stale rõ, failure một source không hỏng nguồn khác. |

## Ownership, dependencies và giới hạn

- Contracts phụ thuộc: Stored articles, Read Later, Notifications; egress conditional. Chỉ truy cập module khác qua contract, không trực tiếp table/DbContext/private store.
- Phạm vi/gate: P-H07: outbound ingestion; chỉ stored/internal reader trước approval.
...
- [Feature/BR/AC nguồn](../../features/29-news-and-feeds.md) — đọc toàn bộ field/state/validation và trace requirements tại đó.
- [Action contracts](../../action-catalog/modules/29-news.md) — exact action keys, contexts, prerequisites, status/gate; catalog có 28 rows: Blocked: 2, Resolved delegated: 26. Đây là inventory, không phải coverage đã pass.
- [UX/screens](../../ux-ui/modules/29-news-feeds.md) — luồng màn hình, disabled/loading/error và interaction contracts.
...
- AC hiện hành cần kế thừa: `FX-29-AC-001`, `FX-29-AC-002`, `FX-29-AC-003`. Cộng source requirement AC, POAC liên quan và [cross-module journeys](../04-cross-module-verification.md); danh sách này không thay test plan đầy đủ.
