...
**Trạng thái:** Specified; chưa approve implementation, chưa Implemented/Verified. Goals có điều kiện không là quyền code. FX-17 phải đọc cùng [goals toàn hệ thống](../01-system-goals.md), [hợp đồng dùng goals](../README.md) và sources hiện hành.

## Mục tiêu và bằng chứng chấp nhận

| Goal ID | Outcome phải giữ khi code | Bằng chứng test tối thiểu khi thực thi |
| --- | --- | --- |
| NXG-FX17-G01 | Habit check-in và streak dựa scheduled local dates. | Unscheduled/paused day không ngắt streak; missed scheduled day ngắt; không check-in tương lai. |
| NXG-FX17-G02 | Target/schedule change giữ lịch sử trước ngày hiệu lực. | Timezone/target mới không tính lại achievements cũ; retry check-in không cộng lặp. |
| NXG-FX17-G03 | Reminder và lifecycle phản ánh thực tế trong ngày. | Đạt target trước dispatch hủy pending alert; Archive read-only; xóa Habit không xóa Tasks/Goals liên kết. |

## Ownership, dependencies và giới hạn

- Contracts phụ thuộc: Clock/timezone, Notifications, Activity. Chỉ truy cập module khác qua contract, không trực tiếp table/DbContext/private store.
...
- [Feature/BR/AC nguồn](../../features/17-habits.md) — đọc toàn bộ field/state/validation và trace requirements tại đó.
- [Action contracts](../../action-catalog/modules/17-habits.md) — exact action keys, contexts, prerequisites, status/gate; catalog có 16 rows: Resolved delegated: 16. Đây là inventory, không phải coverage đã pass.
- [UX/screens](../../ux-ui/modules/17-habits.md) — luồng màn hình, disabled/loading/error và interaction contracts.
...
- AC hiện hành cần kế thừa: `FX-17-AC-001`, `FX-17-AC-002`, `FX-17-AC-003`. Cộng source requirement AC, POAC liên quan và [cross-module journeys](../04-cross-module-verification.md); danh sách này không thay test plan đầy đủ.
