...
**Trạng thái:** Specified; chưa approve implementation, chưa Implemented/Verified. Goals có điều kiện không là quyền code. FX-18 phải đọc cùng [goals toàn hệ thống](../01-system-goals.md), [hợp đồng dùng goals](../README.md) và sources hiện hành.

## Mục tiêu và bằng chứng chấp nhận

| Goal ID | Outcome phải giữ khi code | Bằng chứng test tối thiểu khi thực thi |
| --- | --- | --- |
| NXG-FX18-G01 | Time Tracking có một RunningTimer/User và elapsed time đúng. | Hai tab Start không hai timer; Stop retry một entry; reload/DST không reset hoặc tính bằng giờ hiển thị. |
| NXG-FX18-G02 | Nguồn terminal/Trash dừng timer đúng transition instant. | Project đóng/Task Trash giữ entry evidence, không tiếp tục Task đã khóa. |
| NXG-FX18-G03 | Manual entry và reports không giả net time hoặc tài chính. | Overlap có warning/confirm; reports gross duration; edit có history; không tự ghi Finance, conversion có dedupe. |

## Ownership, dependencies và giới hạn

- Contracts phụ thuộc: Projects/Tasks lifecycle, clock. Chỉ truy cập module khác qua contract, không trực tiếp table/DbContext/private store.
...
- [Feature/BR/AC nguồn](../../features/18-time-tracking.md) — đọc toàn bộ field/state/validation và trace requirements tại đó.
- [Action contracts](../../action-catalog/modules/18-time.md) — exact action keys, contexts, prerequisites, status/gate; catalog có 13 rows: Resolved delegated: 13. Đây là inventory, không phải coverage đã pass.
- [UX/screens](../../ux-ui/modules/18-time-tracking.md) — luồng màn hình, disabled/loading/error và interaction contracts.
...
- AC hiện hành cần kế thừa: `FX-18-AC-001`, `FX-18-AC-002`, `FX-18-AC-003`. Cộng source requirement AC, POAC liên quan và [cross-module journeys](../04-cross-module-verification.md); danh sách này không thay test plan đầy đủ.
