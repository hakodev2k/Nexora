# FX-13 — Calendar, Personal Events và ICS

Product phase: **P02** · Delivery: **RM08** · Phase 8/RM16–RM22 kiểm chứng tích hợp và phát hành.

**Trạng thái:** Specified; chưa approve implementation, chưa Implemented/Verified. Goals có điều kiện không là quyền code. FX-13 phải đọc cùng [goals toàn hệ thống](../01-system-goals.md), [hợp đồng dùng goals](../README.md) và sources hiện hành.

## Mục tiêu và bằng chứng chấp nhận

| Goal ID | Outcome phải giữ khi code | Bằng chứng test tối thiểu khi thực thi |
| --- | --- | --- |
| NXG-FX13-G01 | Personal Event và Task projection giữ authority riêng. | Calendar không sửa Task Event; Personal Completed/Canceled chỉ-đọc, không reopen/share; quá End không tự Complete. |
| NXG-FX13-G02 | Calendar time/all-day và views đúng timezone. | Đổi zone giữ instant/date; DST và end-exclusive all-day đúng; overlap warning không tự cấm Save. |
| NXG-FX13-G03 | ICS import/export theo subset được duyệt. | Skip invalid/duplicate/recurring, bỏ VALARM, report từng entry; export fully-contained, không overlap; Calendar ICS được xuất Personal Events, Task projections hoặc cả hai theo lựa chọn; vẫn cấm standalone Project/Task module export. |

## Ownership, dependencies và giới hạn

- Contracts phụ thuộc: Tasks projection contract, Reminders, ICS/File provider. Chỉ truy cập module khác qua contract, không trực tiếp table/DbContext/private store.
...
- [Feature/BR/AC nguồn](../../features/13-calendar.md) — đọc toàn bộ field/state/validation và trace requirements tại đó.
- [Action contracts](../../action-catalog/modules/13-calendar.md) — exact action keys, contexts, prerequisites, status/gate; catalog có 13 rows: Resolved delegated: 13. Đây là inventory, không phải coverage đã pass.
- [UX/screens](../../ux-ui/modules/13-calendar.md) — luồng màn hình, disabled/loading/error và interaction contracts.
...
- AC hiện hành cần kế thừa: `FX-13-AC-001`, `FX-13-AC-002`, `FX-13-AC-003`, `FX-13-AC-004`. Cộng source requirement AC, POAC liên quan và [cross-module journeys](../04-cross-module-verification.md); danh sách này không thay test plan đầy đủ.
