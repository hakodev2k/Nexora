...
| --- | --- | --- |
| NXG-FX16-G01 | Goal progress phản ánh targets và mẫu số công khai. | Numeric clamp, target>initial; Tasks chỉ Completed vào tử số, Skipped/deleted không giả done; zero targets là No targets. |
| NXG-FX16-G02 | Goal status do owner điều khiển theo lifecycle riêng. | Đạt 100% không tự Complete; reopen Active có history, không áp rule này lên Project. |
| NXG-FX16-G03 | Target links không sở hữu nguồn hoặc tự cộng tiền khác currency. | Xóa Goal không xóa Tasks; bỏ unavailable link mới đổi denominator; savings/Finance cần semantics được duyệt. |

## Ownership, dependencies và giới hạn

- Contracts phụ thuộc: Tasks targets; Finance only for approved monetary scope. Chỉ truy cập module khác qua contract, không trực tiếp table/DbContext/private store.
- Phạm vi/gate: P-H05 nếu Savings Goal cần advanced Finance semantics.
...
- [Feature/BR/AC nguồn](../../features/16-goals.md) — đọc toàn bộ field/state/validation và trace requirements tại đó.
- [Action contracts](../../action-catalog/modules/16-goals.md) — exact action keys, contexts, prerequisites, status/gate; catalog có 22 rows: Resolved delegated: 22. Đây là inventory, không phải coverage đã pass.
- [UX/screens](../../ux-ui/modules/16-goals.md) — luồng màn hình, disabled/loading/error và interaction contracts.
...
- AC hiện hành cần kế thừa: `FX-16-AC-001`, `FX-16-AC-002`, `FX-16-AC-003`. Cộng source requirement AC, POAC liên quan và [cross-module journeys](../04-cross-module-verification.md); danh sách này không thay test plan đầy đủ.
