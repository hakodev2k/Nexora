...
public sealed class M01RuntimeStore
{
    ...
    public M01RuntimeStore(PasswordHashService passwords, SessionCookieService sessions)
    {
        _passwords = passwords;
        _sessions = sessions;
    }

    public M01Result<AcceptedResponse> Register(RegistrationRequest request)
    {
        ...
        lock (_sync)
        {
            ...
            var user = new UserRecord
            {
                ...
                DisplayName = string.IsNullOrWhiteSpace(request.DisplayName)
                    ...
                    : request.DisplayName.Trim()[..Math.Min(request.DisplayName.Trim().Length, RegistrationPolicy.DisplayNameMaxLength)],
    ...
    public M01Result<VerificationResponse> Verify(TokenProofRequest request)
    {
        if (string.IsNullOrWhiteSpace(request.Token))
        {
            return M01Result<VerificationResponse>.Failure("ValidationFailed", StatusCodes.Status422UnprocessableEntity, "Token is required.");
        }
        ...
        lock (_sync)
        {
            ...
            {
                return M01Result<VerificationResponse>.Failure("TokenUnavailable", StatusCodes.Status410Gone, "Token unavailable.");
            }
            ...
            return M01Result<VerificationResponse>.Success(new VerificationResponse("Verified", "EmailVerified", profile));
        }
    }

    public M01Result<AcceptedResponse> ResendVerification(EmailRequest request)
    {
        var normalizedEmail = TryNormalizeEmail(request.Email);
        lock (_sync)
        {
            if (normalizedEmail is not null && _usersByNormalizedEmail.TryGetValue(normalizedEmail, out var user) && user.State == UserState.PendingVerification)
            {
                CaptureToken(user, "EmailVerification", VerificationTtl);
            }
        }
    ...
    public M01Result<LoginIssue> Login(CredentialsRequest request)
    {
        var normalizedEmail = TryNormalizeEmail(request.Email);
        ...
        lock (_sync)
        {
            ...
            {
                return M01Result<LoginIssue>.Failure("InvalidCredentials", StatusCodes.Status401Unauthorized, "Invalid credentials.");
            }
            ...
            if (!decision.Allowed)
            {
                ...
                return M01Result<LoginIssue>.Failure(decision.Code, status, decision.Message);
            }
            ...
            return M01Result<LoginIssue>.Success(new LoginIssue(ToProfile(user), rawHandle, record.AbsoluteExpiresAt));
        }
    }

    public M01Result<object?> Logout(string? rawHandle)
    {
        lock (_sync)
        {
        ...
        return M01Result<object?>.NoContent();
    }

    public M01Result<object?> Reauth(string? rawHandle, PasswordProofRequest request)
    {
        lock (_sync)
        {
            ...
            if (!auth.Succeeded)
            {
                return M01Result<object?>.Failure(auth.Code, auth.StatusCode, auth.Title);
            }

            if (!_passwords.Verify(request.Password, auth.User!.PasswordHash))
            {
                return M01Result<object?>.Failure("InvalidCredentials", StatusCodes.Status401Unauthorized, "Invalid credentials.");
            }
    ...
    public M01Result<AcceptedResponse> RequestPasswordReset(EmailRequest request)
    {
        var normalizedEmail = TryNormalizeEmail(request.Email);
        lock (_sync)
        {
            if (normalizedEmail is not null && _usersByNormalizedEmail.TryGetValue(normalizedEmail, out var user) && !user.IsDeleted)
            {
    ...
    public M01Result<object?> ConfirmPasswordReset(ResetProofRequest request)
    {
        if (string.IsNullOrWhiteSpace(request.Token))
        {
            return M01Result<object?>.Failure("ValidationFailed", StatusCodes.Status422UnprocessableEntity, "Token is required.");
        }

        var passwordDecision = PasswordPolicy.Validate(request.NewPassword);
        if (!passwordDecision.Allowed)
        {
            return M01Result<object?>.Failure("ValidationFailed", StatusCodes.Status422UnprocessableEntity, passwordDecision.Message);
        }
        ...
        lock (_sync)
        {
            ...
            {
                return M01Result<object?>.Failure("TokenUnavailable", StatusCodes.Status410Gone, "Token unavailable.");
            }

            var decision = AccountAccessPolicy.CanConfirmPasswordReset(user.ToSnapshot());
            if (!decision.Allowed)
            {
                return M01Result<object?>.Failure(decision.Code, decision.Code == "MfaRecoveryRequired" ? StatusCodes.Status409Conflict : StatusCodes.Status403Forbidden, decision.Message);
            }
    ...
    public M01Result<ProfileReadResult> GetMe(string? rawHandle)
    {
        lock (_sync)
        {
            var auth = AuthenticateSession(rawHandle);
            if (!auth.Succeeded)
            {
                return M01Result<ProfileReadResult>.Failure(auth.Code, auth.StatusCode, auth.Title);
            }

            return M01Result<ProfileReadResult>.Success(new ProfileReadResult(ToProfile(auth.User!), ETag(auth.User!.RowVersion)));
        }
    }

    public M01Result<ProfileReadResult> UpdateMe(string? rawHandle, string? ifMatch, ProfilePatchRequest request)
    {
        lock (_sync)
        {
            var auth = AuthenticateSession(rawHandle);
            ...
            {
                return M01Result<ProfileReadResult>.Failure("PreconditionRequired", StatusCodes.Status428PreconditionRequired, "If-Match is required.");
            }

            var user = auth.User!;
            if (!string.Equals(ifMatch, ETag(user.RowVersion), StringComparison.Ordinal))
            {
                return M01Result<ProfileReadResult>.Failure("RevisionConflict", StatusCodes.Status412PreconditionFailed, "Resource revision changed.");
            }

            if (request.DisplayName is null && request.TimeZoneId is null && request.Locale is null)
            {
                return M01Result<ProfileReadResult>.Failure("ValidationFailed", StatusCodes.Status422UnprocessableEntity, "At least one allowed field is required.");
            }
            ...
            if (request.DisplayName is { } displayName)
            {
                ...
                {
                    return M01Result<ProfileReadResult>.Failure("ValidationFailed", StatusCodes.Status422UnprocessableEntity, "displayName is invalid.");
                }
            ...
            if (request.TimeZoneId is { } timeZoneId)
            {
                ...
                if (!decision.Allowed)
                {
                    return M01Result<ProfileReadResult>.Failure("ValidationFailed", StatusCodes.Status422UnprocessableEntity, decision.Message);
                }
            ...
            return M01Result<ProfileReadResult>.Success(new ProfileReadResult(ToProfile(user), ETag(user.RowVersion)));
        }
    }

    public M01Result<SessionPage> ListSessions(string? rawHandle)
    {
        lock (_sync)
        {
            var auth = AuthenticateSession(rawHandle);
            if (!auth.Succeeded)
            {
                return M01Result<SessionPage>.Failure(auth.Code, auth.StatusCode, auth.Title);
            }
            ...
            return M01Result<SessionPage>.Success(new SessionPage(sessions, null));
        }
    }

    public M01Result<object?> RevokeSession(string? rawHandle, Guid sessionId)
    {
        lock (_sync)
        {
            var auth = AuthenticateSession(rawHandle);
            ...
            {
                return M01Result<object?>.Failure("ResourceUnavailable", StatusCodes.Status404NotFound, "Session unavailable.");
            }
    ...
    public M01Result<object?> RevokeAll(string? rawHandle)
    {
        lock (_sync)
        {
            var auth = AuthenticateSession(rawHandle);
    ...
    private M01Result<AcceptedResponse>? ValidateRegistration(RegistrationRequest request)
    {
        try
        {
            _ = EmailNormalizer.Normalize(request.Email);
        }
        catch (Exception) when (request.Email is null || request.Email.Length <= 512)
        {
            return M01Result<AcceptedResponse>.Failure("ValidationFailed", StatusCodes.Status422UnprocessableEntity, "Email is invalid.");
        }

        var passwordDecision = PasswordPolicy.Validate(request.Password);
        if (!passwordDecision.Allowed)
        {
            return M01Result<AcceptedResponse>.Failure("ValidationFailed", StatusCodes.Status422UnprocessableEntity, passwordDecision.Message);
        }

        var timeZoneDecision = RegistrationPolicy.ValidateTimeZoneId(request.TimeZoneId);
        if (!timeZoneDecision.Allowed)
        {
            return M01Result<AcceptedResponse>.Failure("ValidationFailed", StatusCodes.Status422UnprocessableEntity, timeZoneDecision.Message);
        }

        if (request.DisplayName is not null && request.DisplayName.Length > RegistrationPolicy.DisplayNameMaxLength)
        {
            return M01Result<AcceptedResponse>.Failure("ValidationFailed", StatusCodes.Status422UnprocessableEntity, "displayName is too long.");
        }

        return null;
    }

    private static M01Result<AcceptedResponse> Accepted() =>
        M01Result<AcceptedResponse>.Success(new AcceptedResponse("Accepted", "CheckEmailIfEligible"), StatusCodes.Status202Accepted, "Accepted");
    ...
    private bool TryFindToken(string rawToken, string purpose, out TokenRecord token, out UserRecord user)
    {
        ...
        if (!_tokensByHash.TryGetValue(hash, out var found) || found.Purpose != purpose || found.ConsumedAt is not null || found.ExpiresAt <= DateTimeOffset.UtcNow)
        {
            return false;
        }
    ...
    private AuthResult AuthenticateSession(string? rawHandle)
    {
        ...
        var now = DateTimeOffset.UtcNow;
        if (session.RevokedAt is not null || session.AbsoluteExpiresAt <= now || session.IdleExpiresAt <= now || session.SecurityStamp != user.SecurityStamp)
        {
            return AuthResult.Fail("AuthenticationRequired", StatusCodes.Status401Unauthorized, "Authentication required.");
        }
    ...
    private static string HashToken(string rawToken) => Convert.ToHexString(SHA256.HashData(System.Text.Encoding.UTF8.GetBytes(rawToken)));
