...
public static class IdentityEndpoints
{
    public static WebApplication MapIdentityEndpoints(this WebApplication app)
    {
        ...
        api.MapPost("/auth/registrations", (HttpContext context, RegistrationRequest request, DevelopmentIdentityStore store) =>
            store.Register(request).ToHttp(context))
            .WithName("register");

        api.MapPost("/auth/verifications", (HttpContext context, TokenProofRequest request, DevelopmentIdentityStore store) =>
            store.Verify(request).ToHttp(context))
            .WithName("verify");

        api.MapPost("/auth/verifications/resend", (HttpContext context, EmailRequest request, DevelopmentIdentityStore store) =>
            store.ResendVerification(request).ToHttp(context))
            .WithName("resendVerification");

        api.MapPost("/auth/login", (HttpContext context, CredentialsRequest request, DevelopmentIdentityStore store, SessionCookieService cookies) =>
        {
        ...
        api.MapPost("/auth/logout", (HttpContext context, DevelopmentIdentityStore store, SessionCookieService cookies) =>
        {
        ...
        api.MapPost("/auth/reauth", (HttpContext context, PasswordProofRequest request, DevelopmentIdentityStore store, SessionCookieService cookies) =>
            store.Reauth(cookies.ReadRawHandle(context.Request), request).ToHttp(context))
            .WithName("reauth");

        api.MapPost("/auth/password-resets", (HttpContext context, EmailRequest request, DevelopmentIdentityStore store) =>
            store.RequestPasswordReset(request).ToHttp(context))
            .WithName("requestReset");

        api.MapPost("/auth/password-resets/confirm", (HttpContext context, ResetProofRequest request, DevelopmentIdentityStore store) =>
            store.ConfirmPasswordReset(request).ToHttp(context))
            .WithName("confirmReset");

        api.MapGet("/me", (HttpContext context, DevelopmentIdentityStore store, SessionCookieService cookies) =>
        {
        ...
        api.MapPatch("/me", (HttpContext context, ProfilePatchRequest request, DevelopmentIdentityStore store, SessionCookieService cookies) =>
        {
        ...
        api.MapGet("/me/sessions", (HttpContext context, DevelopmentIdentityStore store, SessionCookieService cookies) =>
            store.ListSessions(cookies.ReadRawHandle(context.Request)).ToHttp(context))
            .WithName("listSessions");

        api.MapDelete("/me/sessions/{sessionId:guid}", (HttpContext context, Guid sessionId, DevelopmentIdentityStore store, SessionCookieService cookies) =>
            store.RevokeSession(cookies.ReadRawHandle(context.Request), sessionId).ToHttp(context))
            .WithName("revokeSession");

        api.MapPost("/me/sessions/revoke-all", (HttpContext context, DevelopmentIdentityStore store, SessionCookieService cookies) =>
        {
        ...
        {
            api.MapGet("/dev/account-messages", (DevelopmentIdentityStore store) => Results.Ok(store.CapturedMessages()))
